(ns datahike.index.abtset
  #?(:cljs (:require-macros [datahike.index.abtset :refer [generate-slice-comparator-constructor]]))
  (:require [abtset.core :as abtset :refer [IStorage #?@(:cljs [ABTSet Branch ANode Leaf])]]
            [abtset.arrays :as arrays]
  ...
  #?(:clj (:import [datahike.datom Datom]
                   [abtset.core BTSet Branch ANode Leaf]
...
(defn remove-datom> [set ^Datom datom index-type cb]
  (abtset/disj> set datom (index-type->cmp-quick index-type false) cb))

(defn insert> [set ^Datom datom index-type cb]
  (abtset/slice> set
     ...
     (fn [{:as res}]
       (if (contains? res :error)
         ...
         (if (seq (:value res))
           (cb {:value set})
           (abtset/conj> set datom (index-type->cmp-quick index-type) cb))))))

(defn temporal-insert> [set ^Datom datom index-type cb]
  (abtset/conj> set datom (index-type->cmp-quick index-type false) cb))

(defn upsert> [set ^Datom datom index-type old-datom cb]
  (if (nil? old-datom)
    (abtset/conj> set datom (index-type->cmp-quick index-type) cb)
    (remove-datom> set old-datom index-type
       (fn [res]
         (if (contains? res :error)
           (cb res)
           (abtset/conj> (:value res) datom (index-type->cmp-quick index-type) cb))))))

(defn temporal-upsert>
  ;; => {:error E} | {:value btset}
  [btset ^Datom datom index-type {old-val :v} cb]
        ...
        (if (= v old-val)
          ...
          (abtset/conj> btset
                        ...
                        (fn [res]
                          (if (contains? res :error)
                            (cb res)
                            (abtset/conj> (:value res) datom (index-type->cmp-quick index-type false) cb)))))
        (abtset/conj> btset datom (index-type->cmp-quick index-type false) cb))
      (if old-val
        (abtset/conj> btset
                      (dd/datom e a old-val tx false)
                      (index-type->cmp-quick index-type false)
...
(defn mark> [^BTSet btset cb]
  ;; => {:error E} | {:value #{addresses}}
  (when-not (.-_address btset) ;;TODO
    (throw (ex-info "Index needs to be properly flushed before marking."
                    {:type :flush-before-marking})))
  (let [addresses (atom #{})]
    (abtset/walk-addresses> btset (fn [address] (swap! addresses conj address))
      (fn [res]
        (if (contains? res :error)
          (cb res)
          (cb {:value @addresses}))))))
...
(extend-type BTSet
  IIndex
  (-slice [^BTSet btset from to index-type]
